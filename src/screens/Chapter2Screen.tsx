...
function ChapterMap({ progress, onSelectDay, onBuildDay, onReset }: { progress: ChapterProgress; onSelectDay: (day: number) => void; onBuildDay?: (day: number) => void; onReset: () => void }) {
  return <StoreShiftMap chapterId={2} days={chapter2Days} progress={progress} onSelectDay={onSelectDay} onBuildDay={onBuildDay} onReset={onReset} />
}
...
function DayIntro({ dayNumber, onStart, onBack }: { dayNumber: number; onStart: () => void; onBack: () => void }) {
  ...
  return <ShiftIntro chapterId={2} day={day} onStart={onStart} onBack={onBack} />
}
...
function DayResult({ dayNumber, scores, hintCounts, onFinish, onBuild }: { dayNumber: number; scores: number[]; hintCounts: number[]; onFinish: (percent: number, hintsUsed: number) => void; onBuild?: () => void }) {
  const day = chapter2Days.find((item) => item.day === dayNumber)!
  return <ShiftDayResult chapterId={2} day={day} scores={scores} hintCounts={hintCounts} onFinish={onFinish} onBuild={onBuild} />
}

export function Chapter2Screen({ onNavigate }: { onNavigate?: (view: AppView) => void }) {
  const [launchRequest] = useState(() => readShiftLaunch(2, window.sessionStorage))
  useEffect(() => {
