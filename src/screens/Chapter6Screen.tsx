...
import { chapter6ActivityById, chapter6Days } from '../data/chapter6'

const STORAGE_KEY = 'english-shift-chapter6-progress-v1'
...
function ChapterMap({ progress, onSelectDay, onReset }: { progress: ChapterProgress; onSelectDay: (day: number) => void; onReset: () => void }) {
  const unlockedDay = Math.min(36, Math.max(31, 31 + progress.completedDays.length))
  const chapterComplete = progress.completedDays.includes(36)

  return (
    <main className="chapter-map chapter-six">
      <section className="chapter-hero">
        <div>
          <div className="eyebrow">LEVEL 1 · CHAPTER 6</div>
          <h2>Hotel</h2>
          <p>Day 31〜36では、時系列・伝聞・推理・サービス診断・確認表現をホテル業務で統合します。</p>
      ...
      {chapterComplete && (
        <section className="chapter-clear-card">
          ...
          <h2>Hotel Complete</h2>
          <p>予約履歴、reported speech、証拠推理、サービス診断、確認表現、Overbooking IncidentまでをLevel 1で一周しました。</p>
        </section>
      )}

      <details className="chapter-reset-details">
        <summary>Progress options</summary>
        <button className="secondary-button" onClick={onReset}>Reset Chapter 6 progress</button>
      </details>
    </main>
  )
}
...
function DayIntro({ dayNumber, onStart, onBack }: { dayNumber: number; onStart: () => void; onBack: () => void }) {
  const day = chapter6Days.find((item) => item.day === dayNumber)!
  return (
    <main className="chapter-day-intro">
      <button className="chapter-back" onClick={onBack}>← Chapter map</button>
      <div className="eyebrow">CHAPTER 6 · DAY {day.day}</div>
      <h2>{day.title}</h2>
...
function DayResult({ dayNumber, scores, hintCounts, onFinish }: { dayNumber: number; scores: number[]; hintCounts: number[]; onFinish: (percent: number, hintsUsed: number) => void }) {
  ...
  return (
    <main className="chapter-day-result">
      ...
      <button className="primary chapter-start" onClick={() => onFinish(percent, hintsUsed)}>{day.day === 36 ? 'Complete Chapter' : 'Finish Day'}</button>
    </main>
  )
}

export function Chapter6Screen() {
  ...
  const day = selectedDay ? chapter6Days.find((item) => item.day === selectedDay) : undefined
  const activity = day ? chapter6ActivityById(day.activityIds[activityIndex]) : undefined
