...
import { chapter5ActivityById, chapter5Days } from '../data/chapter5'

const STORAGE_KEY = 'english-shift-chapter5-progress-v1'
...
function ChapterMap({ progress, onSelectDay, onReset }: { progress: ChapterProgress; onSelectDay: (day: number) => void; onReset: () => void }) {
  const unlockedDay = Math.min(30, Math.max(25, 25 + progress.completedDays.length))
  const chapterComplete = progress.completedDays.includes(30)

  return (
    <main className="chapter-map chapter-five">
      <section className="chapter-hero">
        <div>
          <div className="eyebrow">LEVEL 1 · CHAPTER 5</div>
          <h2>Restaurant / Café</h2>
          <p>Day 25〜30では、注文・数量・丁寧表現・アレルギー・品切れ・混雑対応を英語で処理します。</p>
      ...
      {chapterComplete && (
        <section className="chapter-clear-card">
          ...
          <h2>Restaurant / Café Complete</h2>
          <p>注文処理、数量、丁寧表現、アレルギー確認、品切れ対応、Dinner RushまでをLevel 1で一周しました。</p>
        </section>
      )}

      <details className="chapter-reset-details">
        <summary>Progress options</summary>
        <button className="secondary-button" onClick={onReset}>Reset Chapter 5 progress</button>
      </details>
    </main>
  )
}
...
function DayIntro({ dayNumber, onStart, onBack }: { dayNumber: number; onStart: () => void; onBack: () => void }) {
  const day = chapter5Days.find((item) => item.day === dayNumber)!
  return (
    <main className="chapter-day-intro">
      <button className="chapter-back" onClick={onBack}>← Chapter map</button>
      <div className="eyebrow">CHAPTER 5 · DAY {day.day}</div>
      <h2>{day.title}</h2>
...
function DayResult({ dayNumber, scores, hintCounts, onFinish }: { dayNumber: number; scores: number[]; hintCounts: number[]; onFinish: (percent: number, hintsUsed: number) => void }) {
  ...
  return (
    <main className="chapter-day-result">
      ...
      <button className="primary chapter-start" onClick={() => onFinish(percent, hintsUsed)}>{day.day === 30 ? 'Complete Chapter' : 'Finish Day'}</button>
    </main>
  )
}

export function Chapter5Screen() {
  ...
  const day = selectedDay ? chapter5Days.find((item) => item.day === selectedDay) : undefined
  const activity = day ? chapter5ActivityById(day.activityIds[activityIndex]) : undefined
