...
import { chapter4ActivityById, chapter4Days } from '../data/chapter4'

const STORAGE_KEY = 'english-shift-chapter4-progress-v1'
...
function ChapterMap({ progress, onSelectDay, onReset }: { progress: ChapterProgress; onSelectDay: (day: number) => void; onReset: () => void }) {
  const unlockedDay = Math.min(24, Math.max(19, 19 + progress.completedDays.length))
  const chapterComplete = progress.completedDays.includes(24)

  return (
    <main className="chapter-map chapter-four">
      <section className="chapter-hero">
        <div>
          <div className="eyebrow">LEVEL 1 · CHAPTER 4</div>
          <h2>Electronics Store</h2>
          <p>Day 19〜24では、商品仕様・丁寧な質問・機能説明・故障診断・スタッフ引継ぎを英語で処理します。</p>
      ...
      {chapterComplete && (
        <section className="chapter-clear-card">
          ...
          <h2>Electronics Store Complete</h2>
          <p>商品仕様の説明、関係節、間接疑問、機能説明、故障診断、スタッフ引継ぎまでをLevel 1で一周しました。</p>
        </section>
      )}

      <details className="chapter-reset-details">
        <summary>Progress options</summary>
        <button className="secondary-button" onClick={onReset}>Reset Chapter 4 progress</button>
      </details>
    </main>
  )
}
...
function DayIntro({ dayNumber, onStart, onBack }: { dayNumber: number; onStart: () => void; onBack: () => void }) {
  const day = chapter4Days.find((item) => item.day === dayNumber)!
  return (
    <main className="chapter-day-intro">
      <button className="chapter-back" onClick={onBack}>← Chapter map</button>
      <div className="eyebrow">CHAPTER 4 · DAY {day.day}</div>
      <h2>{day.title}</h2>
...
function DayResult({ dayNumber, scores, hintCounts, onFinish }: { dayNumber: number; scores: number[]; hintCounts: number[]; onFinish: (percent: number, hintsUsed: number) => void }) {
  ...
  return (
    <main className="chapter-day-result">
      ...
      <button className="primary chapter-start" onClick={() => onFinish(percent, hintsUsed)}>{day.day === 24 ? 'Complete Chapter' : 'Finish Day'}</button>
    </main>
  )
}

export function Chapter4Screen() {
  ...
  const day = selectedDay ? chapter4Days.find((item) => item.day === selectedDay) : undefined
  const activity = day ? chapter4ActivityById(day.activityIds[activityIndex]) : undefined
